...
async function subscribeToNewRaydiumPools(): Promise<void> {
    connection.onLogs(new PublicKey(RAYDIUM_POOL_V4_PROGRAM_ID), async (txLogs: Logs) => {
        ...
        try {
            ...
            try {
                ...
                console.log('Assuming new file will be created:', readError);
            }
            existingData.push(poolKeys);
            await writeFile('mainnet.json', JSON.stringify(existingData, null, 4), 'utf8');
            console.log('poolKeys appended to mainnet.json successfully');
        } catch (error) {
            console.error('Error appending poolKeys to mainnet.json:', error);
        }
    });
